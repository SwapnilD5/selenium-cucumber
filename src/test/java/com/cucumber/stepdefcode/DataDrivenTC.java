package com.cucumber.stepdefcode;

import org.junit.Assert;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.Select;

import io.cucumber.java.After;
import io.cucumber.java.Before;
import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import io.github.bonigarcia.wdm.WebDriverManager;

public class DataDrivenTC {

	WebDriver w;

	@Before
	public void setUp() {
		WebDriverManager.chromedriver().setup();
		w = new ChromeDriver();

	}

	@Given("Open chrome and navigate to calculate page")
	public void open_chrome_and_navigate_to_calculate_page() {

		w.get("https://juliemr.github.io/protractor-demo/");

	}

	@When("Enter two numbers {string} and {string} with operator {string}")
	public void enter_two_numbers_and_with_operator(String firstName, String secondNumber, String operator) {

		w.findElement(By.cssSelector("input[ng-model='first']")).sendKeys(firstName);
		Select sc = new Select(w.findElement(By.cssSelector("select[ng-model='operator']")));
		sc.selectByVisibleText(operator);

		w.findElement(By.cssSelector("input[ng-model='second']")).sendKeys(secondNumber);
		w.findElement(By.id("gobutton")).click();

	}

	@Then("Validate output {string}")
	public void validate_output(String result) throws Exception {

		Thread.sleep(2000);
		String outputValue = w.findElement(By.cssSelector("h2.ng-binding")).getText();
		Assert.assertEquals(result, outputValue);

	}

	@After
	public void tearDown() {

		w.quit();

	}

}
